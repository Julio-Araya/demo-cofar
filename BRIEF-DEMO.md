...
1. **Datos congelados, cero llamadas en vivo.** La página NUNCA llama a `services.cofar.cl` ni a `cofar.cl` en tiempo de ej sale de un JSON generado en build desde `evidencia/`. Un visitante del demo no debe generar ni un request a la infraestructura del cliente.
...
3. **Seis SKUs, no el catálogo.*s una muestra para explicar un punto, no una copia de su tienda.
...
6. **Fechasible.** Pie de página: datos capturados el 2026-09-12 desde fuentes públicas, no en vivo, sin relación comercial con Cofar. En texto legible, no en letra chica.
...
| 6 | Vichy Liftactiv Supreme Vit C Serum 20 ml | Conía mejor descrita (69%) |
...
3. ON-LD actual** — del DOM (`evidencia/p2/jsonld_dom_analisis.txt` y los DOM guardados). Con sus defectos visibles: `"Marca no disponible"`, `availability` fijo en `InStock`, descripción placeholder cuando corresponda.
...
2. **Tres cifras de contexto**, en tarjetas: 3.024 productos con atributos en el 94-100%, 0 de 60 fichas con dato de producto en el HTML del servidor, 0% de descripcente a 69% en dermocosmética.
...
- **Color.** Ink `#16332B` y `#1E4034` para tinta y paneles oscuros. Off-whit, `#fbfbf3`, `#f0f0e2`, `#deded0`. Coral `#FF5A3C` (acción), `#FF7A5E`, `#E5431F`, `#FFE7DF`. Teal `#0E8C7F` (criterio y método), `#0A5F57`, `#7FC9BC`, `#E2F0EC`, `#063B36`.
...
Se ve en teléfono. La reunión puede ser en un notebook, pero igual se## Stack y publicación
...
- [ ] Cero requests a `cofar.c`services.cofar.cl` en la pestaña de red
